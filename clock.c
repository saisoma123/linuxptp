																...
																int id, struct ptp_message *req, int *changed)
{
	...
	case MID_SYNCHRONIZATION_UNCERTAIN_NP:
		...
		case SYNC_UNCERTAIN_TRUE:
			...
			if (c->local_sync_uncertain != mtd->val && stats_get_num_values(c->stats.offset))
...
void clock_update_time_properties(struct clock *c, struct timePropertiesDS tds)
{
	if ((tds.flags ^ c->tds.flags) & (LEAP_61 | LEAP_59))
	{
		pr_info("updating time properties to %s leap second",
						tds.flags & (LEAP_61 | LEAP_59) ? (tds.flags & LEAP_61 ? "insert" : "delete") : "no");
	}
...
static int tee_bmca_ping(void)
{
	...
	if (res != TEEC_SUCCESS)
	{
		pr_notice("TEE PING: InitializeContext failed 0x%x\n", res);
		return 0;
	}
	...
	if (res != TEEC_SUCCESS)
	{
		pr_notice("TEE PING: OpenSession failed 0x%x origin 0x%x\n", res, err_origin);
	...
	if (res == TEEC_SUCCESS)
		pr_notice("TEE PING magic=0x%x\n", op.params[0].value.a);
	else
		pr_notice("TEE PING failed 0x%x origin 0x%x\n", res, err_origin);
...
static int tee_bmca_set_default_ds(struct clock *c)
{
	...
	if (res != TEEC_SUCCESS)
	{
		pr_notice("TEE: InitializeContext failed 0x%x\n", res);
		return 0;
	}
	...
	if (res != TEEC_SUCCESS)
	{
		pr_notice("TEE: OpenSession failed 0x%x origin 0x%x\n", res, err_origin);
		TEEC_FinalizeContext(&ctx);
		return 0;
	}
	...
	pr_notice("TEE: InvokeCommand returned 0x%x\n", res);
	...
	if (res != TEEC_SUCCESS)
	{
		pr_notice("TEE: SET_DEFAULT_DS failed 0x%x origin 0x%x\n", res, err_origin);
		return 0;
	}
	return 1;
}
...
static int tee_bmc_state_decision(struct clock *c, struct port *p, enum port_state *ps_out)
{
	...
	if (res != TEEC_SUCCESS)
	{
		pr_notice("TEE: DECIDE failed 0x%x origin 0x%x\n", res, err_origin);
		return 0;
	}

	*ps_out = (enum port_state)out.decided_state;
	pr_notice("TEE: DECIDE OK -> decided_state=%u (ps_out=%u)\n",
						out.decided_state, *ps_out);
	return 1;
}
